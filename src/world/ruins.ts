...
let stoneMat!: THREE.MeshLambertMaterial;
let darkMat!: THREE.MeshLambertMaterial;
...
function grave(g: THREE.Group, x: number, z: number, ry: number, graveTex: THREE.Texture): void {
  const gy = getGroundHeight(x, z);
...
export function createRuins(scene: THREE.Scene, col: CollisionWorld): Ruins {
  collision = col;
  stoneMat = new THREE.MeshLambertMaterial({ map: makePixelStone() });
  darkMat = new THREE.MeshLambertMaterial({ map: makePixelDarkStone() });
  ...
  column(g, -2.2, 8, true);
  column(g, 2.2, 8, true);
  block(g, 6.0, 0.9, 1.2, 0, 5.35, 8, 0, true);
  block(g, 1.4, 0.7, 1.0, 3.4, 4.6, 8.2, 0.5, false, 0.2);

  // --- Колоннада вдоль тропы ---
  column(g, -2.8, 3, true);
  column(g, 2.8, 3, false);
  column(g, -3.0, -2.5, false);
  column(g, 3.0, -2.5, true);
  column(g, -3.2, -6.5, true);
  column(g, 3.2, -6.5, false);

  // --- Кладбище слева (за сломанным забором) ---
  grave(g, -6.5, 4.5, 0.3, graveTex);
  grave(g, -8.2, 2.2, -0.2, graveTex);
  grave(g, -6.2, 0.0, 0.5, graveTex);
  grave(g, -8.6, -2.6, 0.1, graveTex);
  grave(g, -6.4, -5.0, -0.4, graveTex);
  grave(g, -9.0, -6.8, 0.35, graveTex);
  for (let i = 0; i < 7; i++) {
    const fx = -4.6 - i * 0.85;
    const fz = 6.2 - i * 1.9;
  ...
  block(g, 0.9, 3.4, 7.0, 7.6, 1.7, 2.5, 0, false, 0, true); // восточная
  block(g, 6.5, 2.6, 0.9, 5.0, 1.3, -1.0, 0, false, 0, true); // северная
  block(g, 6.0, 3.8, 0.9, 5.2, 1.9, 5.8, 0, true, 0, true); // южная
  ...
  block(g, 1.0, 5.2, 1.0, 1.9, pgy + 2.6, -21.5, 0, true, 0, true);
  block(g, 4.8, 0.8, 1.1, 0, pgy + 5.4, -21.5, 0, true);

  return {
    altarPos,
    portalPos: new THREE.Vector3(0, pgy, -21.5),
